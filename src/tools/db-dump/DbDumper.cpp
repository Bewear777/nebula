...
Status DbDumper::openDb() {
  ...
  auto spaceFound = std::find(subDirs.begin(), subDirs.end(), folly::stringPrintf("%d", spaceId_));
  if (spaceFound == subDirs.end()) {
    return Status::Error(
        "Space '%s' not found in directory '%s'.", FLAGS_space_name.c_str(), FLAGS_db_path.c_str());
  }
  ...
  if (!status.ok()) {
    return Status::Error(
        "Unable to open database '%s' for reading: '%s'", path.c_str(), status.ToString().c_str());
  }
  db_.reset(dbPtr);
  return Status::OK();
}
...
bool DbDumper::isValidVidLen(VertexID vid) {
  if (!NebulaKeyUtils::isValidVidLen(spaceVidLen_, vid)) {
    std::cerr << "vertex id length is illegal, expect: " << spaceVidLen_ << " result: " << vid
              << "\n";
    return false;
  }
  return true;
...
void DbDumper::run() {
  ...
  uint32_t bitmap = (parts_.empty() ? 0 : 1 << 3) | (vids_.empty() ? 0 : 1 << 2) |
                    (tagIds_.empty() ? 0 : 1 << 1) | (edgeTypes_.empty() ? 0 : 1);
  switch (bitmap) {
    case 0b0000: {
...
void DbDumper::iterates(kvstore::RocksPrefixIter* it) {
  for (; it->valid(); it->next()) {
    ...
    if (NebulaKeyUtils::isTag(spaceVidLen_, key)) {
      ...
      if (FLAGS_mode == "scan") {
        printTagKey(key);
        auto reader = RowReaderWrapper::getTagPropReader(schemaMng_.get(), spaceId_, tagId, value);
        if (!reader) {
      ...
      if (FLAGS_mode == "scan") {
        printEdgeKey(key);
        auto reader =
            RowReaderWrapper::getEdgePropReader(schemaMng_.get(), spaceId_, edgeType, value);
        if (!reader) {
